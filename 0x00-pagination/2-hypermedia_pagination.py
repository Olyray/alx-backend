...
class Server:
    ...
    def get_hyper(self, page: int = 1, page_size: int = 10) -> List[List]:
        """Returns a dictionary of metadata on the dataset"""
        metadict = {}
        gotten_page = self.get_page(page, page_size)
        if not gotten_page:
            metadict['page_size'] = 0
            metadict['next_page'] = None

        else:
            metadict['page_size'] = page_size
            metadict['next_page'] = page + 1
        metadict['page'] = page
        metadict['data'] = gotten_page
        if page == 1:
            metadict['prev_page'] = None
        else:
            metadict['prev_page'] = page - 1
        metadict['total_pages'] = math.ceil(len(self.dataset())/page_size)
        return metadict
